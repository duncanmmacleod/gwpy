...
class TestTimeSeries(_TestTimeSeriesBase):
    ...
    def test_read_write_gwf_error(self, tmp_path, api, losc):
        tmp = tmp_path / "test.gwf"
        ...
        assert str(exc.value).startswith(
            "Failed to read {0!r} from {1!r}".format(losc.name, str(tmp))
        )
    ...
    @pytest.mark.parametrize("api", GWF_APIS)
    def test_write_gwf_type(self, losc, tmp_path, api, ctype):
